...
async def dev_create_session(language: str = "en"):
    ...
    if not available_table:
        raise HTTPException(status_code=400, detail="All tables are currently occupied.")
            
    session_id = f"SES-{uuid.uuid4().hex[:12].upper()}"
    customer_ticket = create_customer_ticket(available_table, session_id)
    ...
    # Broadcast to admin/kitchen that a new session started
    ...
    return {
        "message": f"Dev Session started for table {available_table}",
        "table_number": available_table,
        "session_id": session_id,
        "customer_ticket": customer_ticket,
        "language": language,
